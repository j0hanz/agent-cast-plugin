...
interface FilterState { prototypes: string; screenshots: string }
interface QueryState { prototypes: string; screenshots: string }
interface SegState {
  detail: Device | null;
  sandbox: Device | null;
...
const setFilter = (group: keyof FilterState, val: string) => setState({ filter: { ...state.filter, [group]: val } });
const setQuery = (group: keyof QueryState, val: string) => setState({ query: { ...state.query, [group]: val } });
const setSeg = <K extends keyof SegState>(group: K, val: SegState[K]) => setState({ seg: { ...state.seg, [group]: val } });
...
function setState(patch: Partial<UIState>) { state = { ...state, ...patch }; listeners.forEach(l => l()); }
