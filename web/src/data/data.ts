...
import type { Device, Status, Finding, Screenshot, Stage, TestStatus, TestRunInput, LoopStep } from './types.ts';
...
export const screenshotSrc = (protoId: string, ver: string): string => `/screenshots/${protoId}-${ver}.png`;
...
export const latestScreenshot = (screenshots: Screenshot[]): Screenshot | null =>
  screenshots.reduce((latest: Screenshot | null, s) => (!latest || s.capturedAt > latest.capturedAt) ? s : latest, null);
...
export const AGENT: { running: boolean; stage: string } = new Proxy({ running: false, stage: '' }, {
  get(_target, prop) {
    return Reflect.get(deriveAgent(SCREENSHOTS), prop);
  }
});
export const STATUS_OF: Record<string, Status | null> = { All: null, Live: 'live', Drafts: 'draft', Passed: 'passed', Failed: 'failed' };
export const VIEWPORTS: Record<Device, string> = { Desktop: '1440×900', Tablet: '834×1112', Mobile: '390×844' };
...
export const testSummary = (tests: { status: TestStatus }[]): { passing: number; failing: number; suites: number } => ({
...
export const SCREENSHOT_PROTOS: string[] = new Proxy([] as string[], {
  get(_target, prop) { return Reflect.get(screenshotProtos(), prop); },
  has(_target, prop) { return Reflect.has(screenshotProtos(), prop); }
});
