...
export const AgentPill = memo(({ running, stage }: { running: boolean; stage: string }) => running ? (
...
const SingleToggle = ({ opts, value, onChange, className, itemClass, ariaLabel }: SingleToggleProps) => (
...
interface SegProps { opts: string[]; value: string; onChange: (val: string) => void; 'aria-label'?: string }
export const Seg = memo((p: SegProps) => <SingleToggle {...p} className={styles.seg} ariaLabel={p['aria-label'] || "Select viewport"} />);

interface ChipsProps { labels: string[]; value: string; onChange: (val: string) => void }
export const Chips = memo((p: ChipsProps) => <SingleToggle {...p} opts={p.labels} className={styles.chips} itemClass={styles.chip} />);

interface PanelProps { title: ReactNode; count?: ReactNode; children?: ReactNode; className?: string }
...
interface PreviewProps { id: string; ver?: string; stage?: string }
...
interface EmptyStateProps { icon?: IconName; title?: string; description?: string }
export const EmptyState = memo(({ icon = 'search', title = 'No results found', description = 'Try adjusting your filters.' }: EmptyStateProps) => (
