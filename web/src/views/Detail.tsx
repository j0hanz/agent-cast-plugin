...
const StepRow = memo(({ s }: { s: LoopStep }) => (
  <div className={`${styles.s} ${styles[s.state] ?? ''}`}>
    <span className={styles.dot} />
    <div className={styles.n}>{s.name}</div>
    ...
    return (
      <Main topbar={<p className={styles.crumb}>Prototype not found</p>}>
        <EmptyState
          ...
          description="The requested prototype does not exist."
        />
      </Main>
    );
  }
    ...
    <>
      <nav aria-label="breadcrumb">
        <ol className={styles.crumbList}>
          ...
          <li aria-current="page" className={styles.crumb}>
            {p.name}
          </li>
        </ol>
      </nav>
    ...
    <Main topbar={top}>
      ...
      <div className={`${ui.cols} ${ui.stretch}`}>
        <div className={ui.rail}>
          <Preview
            key={`${p.id}:${versions[versions.length - 1]}`}
            id={p.id}
          ...
          <Panel title="Captured versions" count={`${versions.length} iterations`}>
            <div className={styles.versions}>
              {versions.map((v, i) => {
                ...
                return (
                  <div key={v} className={`${styles.ver} ${curr ? styles.cur : ''}`}>
                    {v}
                    {curr && ' · current'}
                  </div>
                );
        ...
        <div className={ui.stackFill}>
          ...
          <Panel title="Critique findings" count={`${findings.length} open`} className={ui.clip}>
            {findings.length ? (
              findings.map((f, i) => <FindingRow key={`${f.loc}-${i}`} f={f} />)
            ) : (
              <EmptyState
                icon="search"
                title="No findings"
                description="This prototype has no open critique findings."
              />
            )}
          </Panel>
        </div>
