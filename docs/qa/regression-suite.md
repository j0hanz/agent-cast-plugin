...
- Run 2026-07-02: Smoke + Targeted (Detail/Sandbox/System/ui.tsx/data-file mappings), mock mode, against the build that includes commits e4ff78c/9894dc7/1521f2c. 33/33 executed cases passed (TC-NAV-001→004, TC-PROTO-001/005, TC-SC-001, TC-DETAIL-001→011, TC-SANDBOX-001→005/007, TC-TESTS-001/008, TC-SYS-001/003/006/007, TC-DATA-001/005, TC-RESP-001). Zero console errors across every view/route visited; no cross-prototype/cross-version data leak found. Deferred to the live-mode pass (TASK-006): TC-SANDBOX-006, TC-SYS-005, TC-SYS-008 (all documented live-only). Known non-reproducible in mock, unchanged: TC-PROTO-006. Newly observed same-class gap: TC-SYS-004's >60-char truncation branch is not reachable with current mock fixtures (all `MCP_CALLS` args are short) — not a regression, just unexercised, same pattern as TC-PROTO-006. Doc-accuracy note: TC-SYS-002's wording ("Server/Transport/Status/Uptime-or-Recent-activity rows") no longer matches the Server panel's actual fields (Browser/Headless/Viewport/Capabilities/Output dir, per `MCP` in `mock.ts`/`live.ts`) — the panel itself renders correctly, the test case prose is stale and needs a follow-up rewrite. TC-A11Y-001 was spot-checked via confirmed-reachable interactive elements throughout the run rather than one continuous fresh-load Tab trace. No P0 failures; no blockers.
